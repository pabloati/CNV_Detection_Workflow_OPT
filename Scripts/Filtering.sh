...
for i in $(ls *_noSup.bam); do
  name=$(echo $i | cut -d '_' -f 1)
   ~/Programs/samtools-1.9/samtools view -h $i | perl samparser.sh -e 50 --mm 2 50 --insert 150 800 -i - |  ~/Programs/samtools-1.9/samtools view -Sb - > "$name""_noSup_10k_filtered.bam"
  done
